...
function ServicesPlan({ onChange, resetTrigger}) {
  ...
  const updateProductDict = (newSelectedOptions, newSelectedAddons) => {
    ...
    const productDict = selectedCategories.map(category => {
      ...
      // Fix: Calculate priceKey per item based on current total categories selected
      ...
      return {
        ...
        adds_on: paidAddons,
        price: basePrice + paidAddonTotal,
      };
    });
  ...
  const handleSubscriptionChange = (category, subscription) => {
    ...
    if (!subscription) {
      // If empty string is selected, remove this category
    ...
    updateProductDict(updatedOptions, updatedAddons); // ensure final output reflects removal
  };
  ...
  const getSubscriptionCharge = () => {
    ...
    return selectedCategories.reduce((sum, category) => {
      ...
      return sum + parseFloat(subData[priceKey] || 0);
    }, 0);
  };
  ...
  <div className="text-blue-900 text-xl font-bold">
    ...
    {
     (getTotalPrice() + getSubscriptionCharge()).toFixed(2)
